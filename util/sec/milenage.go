...
func (m *Milenage) CheckSqn(maskedsqn []uint8, randv []uint8) (sqn []uint8, macs []uint8, err error) {
	if len(maskedsqn) != 6 {
		err = fmt.Errorf("Wrong input size")
		return
	}

	if err = m.SetRand(randv); err != nil {
		return
	}
	...
	sqn = make([]uint8, 6)
	res, ak := m.F2F5()
	log.Tracef("res=%x, ak=%x", res, ak)
	for i := 0; i < 6; i++ {
		sqn[i] = ak[i] ^ maskedsqn[i]
	}

	log.Tracef("sqn=%x", sqn)
	_, macs, _ = m.F1(sqn, amf[:])

	log.Tracef("macs=%x-%x", macs, amf[:])
	return
}
