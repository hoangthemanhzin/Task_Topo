document.addEventListener("DOMContentLoaded", function () {
  ...
  function changeNodeColor(nodeId) {
    console.log("node change:", nodeId);
    // Tạo một biến để lưu trữ màu sắc cho các node UPF
    const upfColor = 'blue';

    // Lấy SVG container hoặc chọn một container phù hợp của bạn
    const svgContainer = d3.select("#network svg");

    // Cập nhật màu sắc của các node UPF
    svgContainer.selectAll(".node.upf")
        .filter(d => d.id === nodeId)
        .style("fill", upfColor);
  }

  function changeLinkStyle(nodeId) {
    const svgLink = d3.select("#network svg");
  ...
  function handleActionButtonClick(action) {
    if (selectedNode) {
        ...
        changeNodeColor(selectedNode.id);
        // Ham logic sua lai cac link thanh net dut :
        changeLinkStyle(selectedNode.id);
    }
  }
  ...
  function changeNodeColorActive(nodeId) {
    console.log("node change:", nodeId);
    // Tạo một biến để lưu trữ màu sắc cho các node UPF
    const upfColor = 'beige';
   ...
   function changeLinkStyleActive(nodeId) {
    const svgLink = d3.select("#network svg");
    svgLink.selectAll(".link")
      .filter(link => link.source.id === nodeId ||  link.target.id === nodeId)
  ...
  function handleAdditionalButtonClick(buttonId) {
    ...
    // Doi mau khi active
    changeNodeColorActive(buttonId);
    // Doi kieu link khi active
    changeLinkStyleActive(buttonId);
  }
