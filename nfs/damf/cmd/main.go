...
var _nf *service.DAMF
var _logfile *os.File
var log logctx.LogWriter

func main() {

	fmt.Println("B5gc DAMF starts running")

	app := cli.NewApp()
	app.Name = "damf"
	app.Usage = "5G Default Access and Mobility Management Function (DAMF)"
	app.Action = action
	app.Flags = flags
...
func action(c *cli.Context) (err error) {
	...
	//read config
	var cfg config.DamfConfig
	filename := c.String("config")
	if cfg, err = config.LoadConfig(filename); err != nil {
		fmt.Println("Fail to parse DAMF configuration: " + err.Error())
		return
	}
	...
	if _nf, err = service.New(&cfg); err != nil {
		log.Errorf("Fail to create DAMF service: %s", err.Error())
		return
	}
	//create mesh agent
	...
	if err = _nf.Start(); err != nil {
		log.Errorf("Fail to start DAMF service:%s", err.Error())
		return
	}
