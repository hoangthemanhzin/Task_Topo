...
func main() {
	fmt.Println("B5gc PCF starts running")

	app := cli.NewApp()
	app.Name = "pcf"
	app.Usage = "Etri 5G Policy Control Function (PCF)"
	app.Action = action
	app.Flags = flags
...
func action(c *cli.Context) (err error) {
	...
	if cfg, err = config.LoadConfig(filename); err != nil {
		fmt.Println("Fail to parse PCF configuration: " + err.Error())
		return
	}
	...
	if _nf, err = service.New(&cfg); err != nil {
		log.Errorf("Fail to create PCF service: %s", err.Error())
		return
	}
	...
	if err = _nf.Start(); err != nil {
		log.Errorf("Fail to start PCF service: %s", err.Error())
		return
	}
