...
func main() {
	fmt.Println("B5gc SMF starts running")

	app := cli.NewApp()
	app.Name = "smf"
	app.Usage = "Etri 5G Session Management Function (SMF)"
	app.Action = action
	app.Flags = flags
...
func action(c *cli.Context) (err error) {
	...
	if cfg, err = config.LoadConfig(cfgfile); err != nil {
		log.Errorf("Fail to parse SMF configuration: %s", err.Error())
		return
	}
	...
	if _nf, err = service.New(&cfg); err != nil {
		log.Errorf("Fail to create SMF service: %s", err.Error())
		return
	}
	...
	if err = _nf.Start(); err != nil {
		log.Errorf("Fail to start SMF service: %s", err.Error())
		return
	}
