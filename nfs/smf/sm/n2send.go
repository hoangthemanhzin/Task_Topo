...
func (smctx *SmContext) buildPduSessionResourceSetupRequestTransfer() (buf []byte, err error) {
	//anupf := annode.upf
	teid := make([]byte, 4)
	binary.BigEndian.PutUint32(teid, smctx.tunnel.AnTeid())

	transfer := ngapType.PDUSessionResourceSetupRequestTransfer{}

	// PDU Session Aggregate Maximum Bit Rate
	// This IE is Conditional and shall be present
	// when at least one NonGBR QoS flow is being
	// setup.
	// TODO: should check if there is at least
	// one NonGBR QoS flow
	...
	if srule == nil {
		err = fmt.Errorf("No PDU Session AMBR")
		return
	}
	ie.Value = ngapType.PDUSessionResourceSetupRequestTransferIEsValue{
		...
		PDUSessionAggregateMaximumBitRate: &ngapType.PDUSessionAggregateMaximumBitRate{
			PDUSessionAggregateMaximumBitRateDL: ngapType.BitRate{
				Value: ngapConvert.UEAmbrToInt64(srule.AuthSessAmbr.Downlink),
			},
			PDUSessionAggregateMaximumBitRateUL: ngapType.BitRate{
				Value: ngapConvert.UEAmbrToInt64(srule.AuthSessAmbr.Uplink),
			},
		},
	}
	...
	n3ip := smctx.tunnel.AnIp().To4()

	smctx.Tracef("UPF id = %s[%v],teid=%d[%v]", n3ip.String(), n3ip, smctx.tunnel.AnTeid(), teid)
	...
	*/
	if buf, err = aper.MarshalWithParams(transfer, "valueExt"); err != nil {
		err = fmt.Errorf("encode resourceSetupRequestTransfer failed: %s", err)
	}
	return
}

// TS 38.413 9.3.4.9
func (smctx *SmContext) buildPathSwitchRequestAcknowledgeTransfer() (pdu []byte, err error) {
	return
	/*
...
func (smctx *SmContext) buildPathSwitchRequestUnsuccessfulTransfer(causePresent int, causeValue aper.Enumerated) (buf []byte, err error) {
	/*
...
func (smctx *SmContext) buildPduSessionResourceReleaseCommandTransfer() (buf []byte, err error) {
	/*
...
func (smctx *SmContext) buildHandoverCommandTransfer() (buf []byte, err error) {
	/*
		ANUPF := ctx.Tunnel.DataPathPool.GetDefaultPath().FirstDPNode
		UpNode := ANUPF.UPF
		teidOct := make([]byte, 4)
